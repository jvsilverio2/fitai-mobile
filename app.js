...
from "./js/exercise-data.js";


import {
    stabilizePoseLandmarks,
    buildMotionFrame,
    resetMotionEngine
}
from "./js/motion-engine.js";


import {
    setShadowLiveLandmarks,
    clearShadowLiveLandmarks,
    drawExerciseShadow
}
from "./js/shadow-engine.js";
...
function toggleFavorite(id) {
    ...
    if (
        ...
        slowGuideButton.disabled =
            !profile.slowGuide;


        slowGuideButton.textContent =
            profile.slowGuide
                ?
                "▶ VER GUIA LENTO"
                :
                "GUIA LENTO · NÃO CONFIGURADO";
...
function openSlowGuideForExercise(
    exercise,
    onDone = null
) {
    ...
    overlay.innerHTML = `
        ...
        <div class="generic-slow-guide__top">
            <div>
                <small>FITAI · GUIA LENTO</small>
                <h2>${escapeHtml(exercise.name)}</h2>
            </div>
            <button id="generic-slow-guide-close" type="button">×</button>
        </div>

        <div class="generic-slow-guide__stage">
            <canvas id="generic-slow-guide-canvas"></canvas>
        </div>

        <p>
            A referência amarela executa o movimento lentamente para você acompanhar.
        </p>

        <button id="generic-slow-guide-done" class="yellow-button" type="button">
            ${onDone ? "COMEÇAR TREINO" : "ENTENDI"}
        </button>
    ...
    const canvasGuide =
        document.getElementById(
            "generic-slow-guide-canvas"
        );


    const guideCtx =
        canvasGuide.getContext(
            "2d"
        );


    const stage =
        canvasGuide.parentElement;
    ...
    const resizeGuide =
        () => {
            ...
            canvasGuide.style.width =
                `${rect.width}px`;


            canvasGuide.style.height =
                `${rect.height}px`;


            guideCtx.setTransform(
                dpr, 0, 0, dpr, 0, 0
            );
    ...
    .addEventListener(
        ...
        () => {
            ...
            if (
                ...
                firstGuideRow.classList.toggle(
                    "disabled",
                    !profile.slowGuide
                );


                firstGuideToggle.disabled =
                    !profile.slowGuide;


                const seenKey =
                    `fitai_slow_guide_seen_${selectedExercise.id}`;


                firstGuideToggle.checked =
                    profile.slowGuide
                    &&
...
let pendingExerciseDescription =
    "";


let pendingExerciseExecution =
    "";


let pendingExerciseObserves =
    "";


let pendingExerciseFraming =
    "";


let pendingExerciseSlowGuide =
    false;
...
function saveRecordedExercise() {
    ...
    const exercise = {
        ...
        description:
            pendingExerciseDescription,

        execution:
            pendingExerciseExecution,

        observes:
            pendingExerciseObserves,

        framing:
            pendingExerciseFraming,

        slowGuide:
            pendingExerciseSlowGuide
